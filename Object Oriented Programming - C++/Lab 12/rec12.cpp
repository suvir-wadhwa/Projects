...
bool checkMatch(const Node* pattern, const Node* target) {
    while (pattern != nullptr && target != nullptr) {
...
const Node* isSublist(const Node* pattern, const Node* target) {
    ...
    while (curr != nullptr) {
        if (curr->data == pat->data && checkMatch(pat, curr)) {
            return curr;
        }
        else {
            curr = curr->next;
